    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            ForEach(cue.values.indices, id: \.self) { columnIndex in
                NoScrollingTextField(
                    text: $cue.values[columnIndex],
                    textColor: textColorForCell(cue),
                    font: nsFontForCell(columnIndex: columnIndex),
                    alignment: .left,
                    isStrikethrough: cue.isStruckThrough, // Pass strike-through flag
...
struct NoScrollingTextField: NSViewRepresentable {
    ...
    var isStrikethrough: Bool = false // New parameter
