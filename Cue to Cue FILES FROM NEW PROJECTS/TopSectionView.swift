...
struct TopSectionView: View {
    ...
    @State private var targetTimeString: String = "12:00:00" // Default to noon
    @Binding var countdownToTime: Int
    @Binding var countdownToTimeRunning: Bool
    ...
    var body: some View {
        HStack(spacing: 20) {
            ...
            VStack(spacing: 8) {
                settingsButton
                connectionMonitorButton
            }
            .onTapGesture {
                print("🔍 VStack tapped - this should not interfere with buttons")
            }
        }
        .padding()
        ...
        .onAppear {
            // Initialize target date for countdown to time if not already set
            if targetDate == nil {
                targetDate = parseAsTodayTime(targetTimeString)
                print("🔧 onAppear: initialized targetDate to: \(targetDate?.description ?? "nil") from string: '\(targetTimeString)'")
            }
            // Update countdown to time display if target date exists
            if let tDate = targetDate {
                countdownToTime = max(0, Int(round(tDate.timeIntervalSince(Date()))))
                print("🔧 onAppear: countdownToTime set to: \(countdownToTime) seconds")
            }
        }
    ...
    private func tick() {
        ...
        if countdownToTime != newCountdownToTime {
            ...
            updateWebClients() // This is critical for web synchronization!
        }
    ...
    private func startCountdownToTime() {
        guard let tDate = targetDate else { 
            print("❌ startCountdownToTime failed: targetDate is nil")
            return 
        }
        print("✅ startCountdownToTime called with target date: \(tDate)")
        countdownToTimeRunning = true
        // Initialize countdownToTime immediately.
        countdownToTime = max(0, Int(round(tDate.timeIntervalSince(Date()))))
        print("✅ countdownToTime set to: \(countdownToTime) seconds")
        updateWebClients()
    }
    ...
    private var connectionMonitorButton: some View {
        Button(action: { 
            print("🔌 Network button tapped! Current state: \(showConnectionMonitor)")
            showConnectionMonitor.toggle()
            print("🔌 Network button state changed to: \(showConnectionMonitor)")
        }) {
