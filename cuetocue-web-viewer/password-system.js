...
class PasswordSystem {
    ...
    showContent() {
        ...
        // Initialize existing CueToCueViewer ONLY after password check
        if (!window.cueViewer) {
            try {
                window.cueViewer = new CueToCueViewer();
            } catch (error) {
...
// Initialize password system when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    // Only initialize if password system is available
    if (typeof PASSWORD_CONFIG !== 'undefined') {
        window.passwordSystem = new PasswordSystem();
    } else {
        console.warn('Password system not available, showing content without protection');
        // Fallback - show content if password system fails
        const mainContent = document.getElementById('mainContent');
        if (mainContent) {
            mainContent.style.display = 'block';
        }
